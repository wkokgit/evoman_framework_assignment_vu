...
def deap_generalist_twopoint(experiment_name, enemies_in_group, iteration_number):
...
def startup_twopoint():
    ...
    for group_number, enemies_in_group in enemy_groups.items():
        ...
        experiment_name = "deap_generalist_cxTwoPoint/group" + str(group_name)

        for current_run in range(1,11):
            print("----------TWOPOINT------------ RUN " + str(current_run) + " ----------- GROUP " + str(group_name) + "--------------------------")
            iteration_number = iteration_number + 1
            experiment_name_temp = experiment_name + "/" + str(current_run)
            deap_generalist_twopoint(experiment_name_temp, enemies_in_group,iteration_number)
